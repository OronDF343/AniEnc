...
public class ConvertCommand : ICommand
{
    ...
    public async ValueTask ExecuteAsync(IConsole console)
    {
        ...
        if (Resize != null)
        {
            if (Resize.Length is not 1 and not 2)
            {
...
#pragma warning restore CliFx_SystemConsoleShouldBeAvoided // Avoid calling `System.Console` where `CliFx.Infrastructure.IConsole` is available
        ...
        if (Resize != null)
        {
            ...
            foreach (var frame in frames)
            {
                ...
                var width = (uint) Math.Round(resizeWidth);
                var height = (uint) Math.Round(resizeHeight);
                if (isWidthPercent)
                {
                    width = (uint) Math.Round(frame.Width * resizeWidth / 100.0);
                }
                if (isHeightPercent)
                {
                    height = (uint) Math.Round(frame.Height * resizeHeight / 100.0);
                }
    ...
    private static (double dimension, bool isPercent) ParseDimension(string v)
    {
